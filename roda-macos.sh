...
then
    ...
    do
        ...
        saida=`gtime -o /dev/stdout -a -f %e --quiet gtimeout -s SIGINT --preserve-status -k $TTOL $TLIM_BNB ./bnb "$dir_original$dir_instancias$inst" | tee /dev/tty | tail -5`
...
then
    ...
    do
        ...
        saida=`gtime -o /dev/stdout -a -f %e --quiet gtimeout -s SIGINT --preserve-status -k $TTOL $TLIM_HEUR ./heur "$dir_original$dir_instancias$inst" | tee /dev/tty | tail -3`
        num_linhas=`echo "$saida" | wc -l`
        if [ "$num_linhas" != 3 ]
        then
            (>&2 echo "Formato incorreto de saida para a instancia $inst")
...
then
    ...
    do
        ...
        saida=`gtime -o /dev/stdout -a -f %e --quiet ./pli-solver "$dir_original$dir_instancias$inst" | tee /dev/tty | tail -6`
        num_linhas=`echo "$saida" | wc -l`
        if [ "$num_linhas" != 6 ]
        then
            (>&2 echo "Formato incorreto de saida para a instancia $inst")
